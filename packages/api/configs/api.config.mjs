/*
 * Runtime mirror of `src/api.config.ts` for production builds.
 *
 * The TypeScript source is the canonical implementation; this file must stay
 * in 1:1 signature and behaviour parity with it. Keep this file in sync when
 * either `api.config.ts` changes or this file's complementary consumers
 * (see `src/repositories/booking.ts` and `src/trpc/admin/slimfactInvoice.ts`)
 * start passing new optional parameters.
 *
 * Notes:
 *  - No TypeScript types / type-only imports.
 *  - Optional cancellation context (`bookingStatus`, `lastApprovedBooking`,
 *    `ctx`) is accepted but tolerated when absent — existing callers that
 *    pre-date the cancellation refactor still work.
 *  - `dateFns` is destructured from the caller-provided object — runtime
 *    callers (e.g. `repositories/booking.ts`) pass an explicit subset.
 *  - English defaults for line descriptions; `ctx.lang?.booking.cancelationCosts`
 *    overrides when supplied.
 */

const DEFAULT_SURCHARGE_HOLIDAYS = [
  { rule: '01-01' },
  { rule: 'easter' },
  { rule: 'easter 1' },
  { rule: 'easter 39' },
...
const bookingCancelationHandler = ({
  ...
  const isWithinIntervalFn = isWithinInterval
...
const bookingCostsHandler = ({
  ...
  if (dateHolidays && surchargeHolidays.length > 0) {
    ...
    for (const holiday of localHolidays) {
      ...
      if (
        !isWithinIntervalFn(holidayDate, {
          start: bookingStart,
          end: bookingEnd
        })
      ) {
  ...
  for (const vacation of vacations) {
    ...
    const holidaysInVacation = surchargeHolidayDateKeys.filter((dateKey) => {
      ...
      return isWithinIntervalFn(d, {
        start: vacationStart,
        end: vacationEnd
      })
    }).length

    const startInVacation = isWithinIntervalFn(bookingStart, {
      start: vacationStart,
      end: vacationEnd
    })
    const endInVacation =
      startDate !== endDate &&
      isWithinIntervalFn(bookingEnd, {
        start: vacationStart,
        end: vacationEnd
      })
  ...
  if (
    ...
    // Full cancellation — replace lines with the cancelation-cost line(s).
